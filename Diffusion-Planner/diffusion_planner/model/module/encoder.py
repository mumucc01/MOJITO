...
class Encoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # static objects
        static = inputs['static_objects']  #[B,5,10]

        # vector maps
        lanes = inputs['lanes']
        lanes_speed_limit = inputs['lanes_speed_limit'] #[1,70,1]
        lanes_has_speed_limit = inputs['lanes_has_speed_limit']  #[B,70,1]

        B = neighbors.shape[0] #infer:1 train:BZ

        encoding_neighbors, neighbors_mask, neighbor_pos = self.neighbor_encoder(neighbors) ##encoding_neighbors:[1,32,192]
        encoding_static, static_mask, static_pos = self.static_encoder(static) #[1,5,192]
        encoding_lanes, lanes_mask, lane_pos = self.lane_encoder(lanes, lanes_speed_limit, lanes_has_speed_limit) # encoding_lanes [1,70,192]
