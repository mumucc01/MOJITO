...
def get_args():
    ...
    parser.add_argument('--name', type=str, help='log name (default: "diffusion-planner-training")', default="try")
    parser.add_argument('--save_dir', type=str, help='save dir for model ckpt', default="/mnt/volumes/base-3da-ali-sh-mix/chengzhijing/AD-Eccv/DiffusionPlanner_v37/Diffusion-Planner/training_log")
    ...
    parser.add_argument('--normalization_file_path', default='/mnt/volumes/base-3da-ali-sh-mix/chengzhijing/AD-Eccv/DiffusionPlanner_v37/Diffusion-Planner/normalization.json', help='filepath of normalizaiton.json', type=str)
    ...
    parser.add_argument('--save_utd', type=int, help='save frequency', default=10)
    parser.add_argument('--batch_size', type=int, help='batch size (default: 2048)', default=4)
    parser.add_argument('--learning_rate', type=float, help='learning rate (default: 5e-4)', default=5e-4)
    # parser.add_argument('--learning_rate', type=float, help='learning rate (default: 5e-4)', default=3.5e-4)
    ...
    parser.add_argument('--alpha_agent_loss', type=float, help='coefficient of agent loss (default: 1.0)', default=1.0)
    parser.add_argument('--alpha_planning_loss', type=float, help='coefficient of planning loss (default: 1.0)', default=1.0)
    parser.add_argument('--query_len', type=float, help='scene_query_len', default=107) 
    ...
    # decoder
    parser.add_argument('--predicted_neighbor_num', type=int, help='number of neighbor agents to predict', default=0)
    #parser.add_argument('--resume_model_path', type=str, help='path to resume model', default="/mnt/volumes/base-3da-ali-sh-mix/chengzhijing/DiffusionPlanner_tem_czj/Diffusion-Planner/checkpoints/model.pth")
    ...
    parser.add_argument('--port', default='22324', type=str, help='port')
...
def model_training(args):
    ...
    if global_rank == 0:
        ...
        if args.resume_model_path is not None:
            ...
            save_path = f"{args.save_dir}/{args.name}/{time}/"
    ...
    # set up data loaders #args.agent_num:32  args.predicted_neighbor_num:10  args.future_len:80
    ...
    train_loader = DataLoader(train_set, sampler=train_sampler, batch_size=batch_size//ddp.get_world_size(), num_workers=10, pin_memory=args.pin_mem, drop_last=True)
    ...
    diffusion_planner = diffusion_planner.to(local_rank if args.device == 'cuda' else args.device)
    ...
    image_backbone = diffusion_planner.module.image_backbone
    for param in image_backbone.parameters():
        param.requires_grad_(False)
    

    #encoder_dp = diffusion_planner.module.encoder_dp
    #for param in encoder_dp.parameters():
    #    param.requires_grad_(False)
 

    trainable_params = filter(lambda p: p.requires_grad, ddp.get_model(diffusion_planner, args.ddp).parameters())
    ...
    if args.resume_model_path is not None:
        ...
        backbone_state_dict = torch.load(image_backbone_ckpt, map_location=args.device)['state_dict']
        backbone_state_dict = {
        ...
        image_backbone_missing_keys, image_backbone_unexpected_keys = image_backbone.load_state_dict(backbone_state_dict, strict=False)
        print(f"bev encoder missing keys are {image_backbone_missing_keys}")
        print(f"bev encoder unexpected key are {image_backbone_unexpected_keys}")
        init_epoch = 0
        wandb_id = None

    # for name, param in diffusion_planner.module.named_parameters():
    ...
    for epoch in range(init_epoch, train_epochs):
        ...
        train_loss, train_total_loss = train_epoch(train_loader, diffusion_planner, optimizer, args, model_ema, epoch, aug)
