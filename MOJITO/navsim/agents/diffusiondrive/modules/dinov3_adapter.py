...
class SpatialPriorModule(nn.Module): #adapter
    ...
    def forward(self, x, layer_idx):
        ...
        with torch.autocast("cuda", torch.bfloat16):
            ...
            all_layers = self.backbone.get_intermediate_layers(
                x, layer_idx, n=self.interaction_indexes, return_class_token=False
            ) #[B,N,D]
        return all_layers
        '''
        ...
        outs = list()
        for i, layer in enumerate(self.interactions): #adapter
            x, cls = all_layers[i]
            _, c, _ = layer(
                x,
                c,
