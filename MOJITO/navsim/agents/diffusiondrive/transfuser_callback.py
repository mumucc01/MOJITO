...
class TransfuserCallback(pl.Callback):
    ...
    def on_validation_epoch_end(self, trainer: pl.Trainer, lightning_module: pl.LightningModule) -> None:
        ...
        for idx_plot in range(1):
            ...
            #save_image(grid, f"/lpai/volumes/base-3da-ali-sh-mix/chengzhijing/Diffusion_Drive/visualization/val_plot_{idx_plot}_epoch{trainer.current_epoch}.png")
    ...
    def _visualize_model(
        ...
        B = camera.shape[0]
        K = self._num_rows * self._num_columns

        replace = B < K
        sample_indices = np.random.choice(B, size=K, replace=replace)
        plots = []
    ...
    def pdm_score_visualize_model(
        ...
        :return: image tiles as RGB tensors
        """
        camera = features["camera_feature"].unsqueeze(0).permute(0, 2, 3, 1).numpy()
        bev = targets["bev_semantic_map"].numpy()
        lidar_map = features["lidar_feature"].squeeze(0).numpy()
        agent_labels = targets["agent_labels"].numpy()
        agent_states = targets["agent_states"].numpy()
        trajectory = targets["trajectory"].numpy() #[40,3]

        #pred_bev = predictions["bev_semantic_map"].argmax(1).numpy()
        #pred_agent_labels = predictions["agent_labels"].sigmoid().numpy()
        #pred_agent_states = predictions["agent_states"].numpy()
        pred_trajectory = predictions["trajectory"].squeeze(0).numpy()

        
        agent_states_ = agent_states[0][agent_labels[0]]
        ...
        img = torch.from_numpy(lidar_rgb.copy()).permute(2,0,1)  # [3, H, W], uint8
        return img
...
def lidar_map_to_rgb(
    ...
    for color, agent_state_array in zip([gt_color], [agent_states]):
...
def debug_lidar_map_to_rgb(
    ...
    height, width = lidar_map.shape[:2]
    canvas = np.zeros_like(lidar_map)
    lidar_map = canvas
    def coords_to_pixel(coords):
        """Convert local coordinates to pixel indices."""
        pixel_center = np.array([[height / 2.0, width / 2.0]])
