...
class PointcloudEncoder(nn.Module):
    def __init__(self, point_transformer, args):
        ...
        self.trans_dim = args.pc_feat_dim # 384
        self.embed_dim = args.embed_dim # 1024
        self.group_size = args.group_size # 64
    ...
    def forward(self, pts, colors):
        ...
        _, center, features = self.group_divider(pts) #output:[B,512,64,6]
        ...
        x = self.visual.pos_drop(x) #[B,512,384]

        # ModuleList not support forward
        for i, blk in enumerate(self.visual.blocks):
            x = blk(x)
        x = self.visual.norm(x[:, 0, :]) #[B,512,384]-->[B,384]
    ...
    # def forward(self, pts, colors):
    #     # divide the point cloud in the same form. This is important
    #     _, center, features = self.group_divider(pts, colors) #output:[B,512,64,6]
    ...
    #     cls_pos = self.cls_pos.expand(group_input_tokens.size(0), -1, -1)   #[B,1,384]
    #     # add pos embedding
    #     pos = self.pos_embed(center) #[B,512,384]
    #     # final input
    #     x = torch.cat((cls_tokens, group_input_tokens), dim=1) #[B,513,384]
    ...
    #     x = self.visual.pos_drop(x) #[B,513,384]
