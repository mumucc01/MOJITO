...
class EvalConfig:
    ...
    batch_size: Optional[int] = None  # batch size for evaluation, None to use train batch size
...
def eval_knn(
    *,
    ...
    for try_ in train_data_dict.keys():
        ...
        ks = sorted({el if el < len(train_features) else len(train_features) for el in knn_config.ks})
