...
class DinoVisionTransformer(nn.Module):
    ...
    def _get_intermediate_layers_not_chunked(self, x, n, layer_idx) -> list[Tensor]:
        # n = [0, 4, 8, 11] 
        ...
        if layer_idx >= 1:
            ...
            #output, total_block_len = [], len(self.blocks) #[], 12
            ...
            for i in range(start+1, end+1):
                blk = self.blocks[i]
                if self.rope_embed is not None:
                    rope_sincos = self.rope_embed(H=self.H, W=self.W)
                else:
                    rope_sincos = None
                x = blk(x, rope_sincos)
    ...
    def get_intermediate_layers(
        ...
        *,
        ...
        outputs = self._get_intermediate_layers_not_chunked(x, n, layer_idx)
        ...
        if norm:
            ...
            for out in outputs:
                if self.untie_cls_and_patch_norms:
                    ...
                    x_norm_cls_reg = self.cls_norm(out[:, : self.n_storage_tokens])
                    x_norm_patch = self.norm(out[:, self.n_storage_tokens:])
        ...
        outputs = [out[:, self.n_storage_tokens :] for out in outputs]
